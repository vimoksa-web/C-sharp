namespace Game8_10
{
    ...
    internal class Program
    {
        ...
        #region 开始界面逻辑 和 结束场景逻辑
        static void BeginorEndScene(int width, int heigth, ref E_NowScene nowScene)
        {
            //设置初始色
            Console.ForegroundColor = ConsoleColor.White;
            Console.SetCursorPosition(width / 2 - 4, 8);
            Console.WriteLine(nowScene == E_NowScene.Begin ? "飞 行 棋" : "游戏结束");
            //表示当前选中的选项
            int nowSelect = 0;
            //定义一个变量用来退出循环
            bool isExit = false;
            while (true)
            {
                ...
                Console.SetCursorPosition(nowScene == E_NowScene.Begin ? width / 2 - 4 : width / 2 - 5, 15);
                Console.ForegroundColor = nowSelect == 0 ? ConsoleColor.Red : ConsoleColor.White;
                Console.WriteLine(nowScene == E_NowScene.Begin ? "开始游戏" : "回到主界面");
                ...
                switch (Console.ReadKey(true).Key)
                {
                    ...
                    case ConsoleKey.J:
                        if (nowSelect == 0)
                        {
                            nowScene = nowScene == E_NowScene.Begin ? E_NowScene.Game : E_NowScene.Begin;
                            isExit = true;
                        }
                        else
                        {
            ...
            while (true)
            {
                switch (nowScene)
                {
                    ...
                    case E_NowScene.Begin:
                        ...
                        //开始界面逻辑
                        BeginorEndScene(width, height, ref nowScene);
